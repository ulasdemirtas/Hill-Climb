...
#Step 4: Explore Neighbors / generates neighbors by flipping the inclusion status of k items 
def generate_neighbors(solution, k=2):
    neighbors = []
    # Generate neighbors by flipping k items at a time
    for _ in range(len(solution)):

        neighbor = solution.copy()
        # Randomly select k unique indices to flip
        indices_to_flip = random.sample(range(len(solution)), k)

        for index in indices_to_flip:
            neighbor[index] = 1 - neighbor[index]
        neighbors.append(neighbor) #a list of neighbors that differ by multiple items, based on the value of k
    return neighbors
